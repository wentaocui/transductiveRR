...
if __name__ == '__main__':
    ...
    if params.method in ['maml', 'maml_approx']: #maml do not support testing with feature
        ...
        novel_file = os.path.join( checkpoint_dir.replace("checkpoints","features"), split_str +"_orig.hdf5") #defaut split = novel, but you can also test base or val classes
        ...
        for i in range(iter_num):
            ...
            if (i+1) % 100 == 0:
                print('%d/%d Test Acc = %4.2f%% +- %4.2f%%' % (i+1, iter_num, np.mean(np.asarray(acc_all)), 1.96 * np.std(np.asarray(acc_all)) / np.sqrt(iter_num)))
