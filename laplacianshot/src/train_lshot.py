...
# from src.utils.configuration import write_log, set_args_shortcut
# from src.transduction import transductive_encode
# from src.utils import configuration
# from src.lshot_update import bound_update
# from src import datasets
# from src import models
# from src.utils.visualization import visualize

from utils.configuration import write_log, set_args_shortcut
from transduction import transductive_encode
...
def main():
    ...
    if args.data_for_all is not None and args.arch_for_all is not None:
        args = set_args_shortcut(args)
...
def extract_feature(train_loader, val_loader, model, tag='last', tune=False):
    # return out mean, fcout mean, out feature, fcout features
    save_dir = '{}/{}/{}'.format(args.save_path, tag, args.enlarge)
    save_name = '/output_train_%s.plk' % (args.eval_used_set)
    if tune == True:
        save_name= '/output_train_val.plk'

    if os.path.isfile(save_dir + save_name):
        data = load_pickle(save_dir + save_name)
        return data
    else:
    ...
    with torch.no_grad():
        # get training mean
        if not os.path.isfile(save_dir + '/output_train_mean.plk'):
            ...
            if len(fc_out_mean) > 0:
                ...
                fc_out_mean = -1
            save_pickle(save_dir + '/output_train_mean.plk', [out_mean, fc_out_mean])
        else:
            out_mean, fc_out_mean = load_pickle(save_dir + '/output_train_mean.plk')
        ...
        save_pickle(save_dir + save_name, all_info)
        return all_info

def extract_feature_tune(train_loader, val_loader, model, tag='best'):
    # return out mean, fcout mean, out feature, fcout features
    save_dir = '{}/{}/{}'.format(args.save_path, tag, args.enlarge)
    save_name = '/output_tune.plk'
    if os.path.isfile(save_dir + save_name):
        data = load_pickle(save_dir + save_name)
        return data
    else:
    ...
    with torch.no_grad():
        ...
        if not os.path.isfile(save_dir + '/output_train_mean.plk'):
            ...
            out_mean = load_pickle(save_dir + '/output_train_mean.plk')[0]
...
def meta_evaluate(data, train_mean, shot):
    ...
    for _ in warp_tqdm(range(args.meta_test_iter)):
        ...
        if args.enable_transduct == True:
            train_data, test_data = transductive_encode(train_data, test_data, args)
...
def tune_lambda(train_loader, model, log, eval_shot):
    val_loader = get_dataloader('val', aug=False, shuffle=False, out_name=False)
    load_checkpoint(model, 'best')

    out_mean, _, out_dict, _ = extract_feature(train_loader, val_loader, model, tag='best', tune=True)
    ...
    for lmd in lmd_list:
        args.lmd = lmd
        accuracy_info = meta_evaluate_tune(out_dict, out_mean, eval_shot)

        acc_val = accuracy_info[0]
        acc_val_list.append(acc_val)

        print(
            'validation lmd={:0.2f}: Best\nfeature\tCL2N\n{}\t{:.4f}({:.4f})\n'.format(args.lmd,
            'GVP {}Shot'.format(eval_shot), *accuracy_info))
        log.info(
            'validation lmd={:0.2f}: Best\nfeature\tCL2N\n{}\t{:.4f}({:.4f})\n'.format(args.lmd,
            'GVP {}Shot'.format(eval_shot), *accuracy_info))

    acc_val_list = np.asarray(acc_val_list)
    best_acc = acc_val_list.max()
    best_lmd = lmd_list[acc_val_list.argmax()]

    print('Best lambda on validation:\n{:0.2f} with {} shot acc {:.4f}\n'.format(best_lmd, eval_shot, best_acc))
...
def metric_class_type(gallery, query, support_label, test_label, shot, train_mean=None, norm_type='CL2N'):
    ...
    if args.lshot and args.lmd!=0:
        ...
        acc = lshot_prediction(args, knn, lmd, query, unary, support_label, test_label)*100
    else:
...
def do_extract_and_evaluate(model, log):
    ...
    val_loader = get_dataloader(args.eval_used_set, aug=False, shuffle=False, out_name=False)
    ...
    if args.tune_lmd:
        print('Tuning Lambda')
        best_lmd = tune_lambda(train_loader, model, log, args.eval_shot)
    else:
        best_lmd = args.lmd

    args.lmd = best_lmd
    log.info(' Run with lambda {} for {} shot'.format(args.eval_shot, args.lmd))
    accuracy_info = meta_evaluate(out_dict, out_mean, args.eval_shot)

    print(
        'Meta Test: BEST\nfeature\tUN\tL2N\tCL2N\n{}shot\t{:.2f}+-{:.2f}\t{:.2f}+-{:.2f}\t{:.2f}+-{:.2f}\n\t'.format(args.eval_shot, *accuracy_info))
    log.info(
        'Meta Test: BEST\nfeature\tUN\tL2N\tCL2N\n{} shot\t{:.2f} +- {:.2f}\t{:.2f} +- {:.2f}\t{:.2f} +- {:.2f}\n\t'.format(args.eval_shot, *accuracy_info))
    write_log(args, args.eval_shot, accuracy_info, best_lmd)
