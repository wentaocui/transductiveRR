...
def main():
    ...
    if args.evaluate:
        accuracy_info = do_extract_and_evaluate(model, log, args)
    ...
    if args.do_meta_train:
        ...
        print('protypical training')
    else:
        print('NOT protypical training')
...
def extract_feature(train_loader, val_loader, model, args, tag='last'):
    ...
    save_name = '/output_%s.plk' %(args.eval_used_set)
...
def meta_evaluate(data, train_mean, shot):
    ...
    for _ in warp_tqdm(range(args.meta_test_iter)):
        ...
        if args.enable_transduct == True:
            if args.iterative_transduct == True:
                train_data, test_data = iterative_transductive_encode(train_data, test_data, args)
            else:
...
def do_extract_and_evaluate(model, log, args):
    ...
    out_mean, fc_out_mean, out_dict, fc_out_dict = extract_feature(train_loader, val_loader, model, args, 'best')
    accuracy_info = meta_evaluate(out_dict, out_mean, args.eval_shot)
    print(
        'Meta Test: BEST\nfeature\tUN\tL2N\tCL2N\n{}shot\t{:.2f}+-{:.2f}\t{:.2f}+-{:.2f}\t{:.2f}+-{:.2f}\n\t'.format(
            args.eval_shot, *accuracy_info))
    log.info(
        'Meta Test: BEST\nfeature\tUN\tL2N\tCL2N\n{} shot\t{:.2f} +- {:.2f}\t{:.2f} +- {:.2f}\t{:.2f} +- {:.2f}\n\t'.format(
            args.eval_shot, *accuracy_info ))
    write_log(args, args.eval_shot, accuracy_info)
    return accuracy_info
