...
from io_utils import model_dict, parse_args, get_resume_file, get_baseline_file
import random
...
def train(base_loader, val_loader, model, optimization, start_epoch, stop_epoch, params):    
    if optimization == 'Adam':
        optimizer = torch.optim.SGD(model.parameters(), lr=params.adamlr)
    ...
    for epoch in range(start_epoch,stop_epoch):
        ...
        if acc > max_acc : #for baseline and baseline++, we don't use validation here so we let acc = -1
            ...
            max_acc = acc
            outfile = os.path.join(params.checkpoint_dir, 'baseline_model.tar')
...
if __name__=='__main__':
    ...
    elif params.method in ['protonet','transductProtoNet','matchingnet','transductMatchingNet','relationnet','transductRelationNet','relationnet_softmax', 'maml', 'maml_approx']:
